...
#Se corrio el programa varias veces para encontrar el orden de magnitud de cada variable, de acuerdo a esto, sera necesario dar pasos de diferente longitud para cada una.
A_walk = np.append(A_walk, 10**15)
B_walk = np.append(B_walk,(1500))
E_zero_walk = np.append(E_zero_walk, 10**4)
alpha_walk = np.append(alpha_walk, 100)
sigma_walk = np.append(sigma_walk,-3)
...
for i in range(n_steps - 1):
    ...
    A_prime = np.random.normal(A_walk[i], 10**12)
    B_prime = np.random.normal(B_walk[i], 10)
    E_zero_prime = np.random.normal(E_zero_walk[i], 10)
    alpha_prime = np.random.normal(alpha_walk[i], 0.01)
    sigma_prime = np.random.normal(sigma_walk[i], 0.001)
...
print "El valor de E0 es", best_E_zero
